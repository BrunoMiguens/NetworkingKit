...
extension NetworkingTarget {
    ...
    public func urlRequest(for headers: NetworkingHeaderDictionary? = nil) -> NKResult<URLRequest> {

        let result = construct()

        guard let url = URL(string: result.path), !urlString.isEmpty else {
            return NKResult.Failure(.invalidUrl)
        }
        ...
        return NKResult.Success(request)
...
extension NetworkingTarget {
    ...
    fileprivate func constructPost(with parameters: NKStringDictionary, encode: Bool = true) -> NKPostTuple {
        ...
        return (parameters, NKResult.Success(data))
    }
