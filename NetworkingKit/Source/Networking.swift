...
public class Networking {
    ...
    ///   - completion: This is a typical completion handler based on 'NKResult' enum.
    public class func perform(for router: NetworkingTarget, headers: NetworkingHeaderDictionary? = nil, runningTests: Bool = false, completion: @escaping NKHandler) {
        let result = router.urlRequest(for: headers)

        guard let request = result.value, result.isSuccess else {
            completion(NKResult.Failure(result.error))
            return
        }
...
extension Networking {
    ...
    ///   - completion: Handler to dispatch the logic result with a '.Success(T)' or an '.Failure(NKError)'.
    fileprivate class func deserialize(response: URLResponse?, with object: Any?, and error: Error?, to completion: @escaping NKHandler) {

        if let obj = object, obj is NKArray || obj is NKDictionary {
            completion(.Success(obj))
        } else {
            completion(.Failure(NKError.construct(with: error, and: response)))
        }
